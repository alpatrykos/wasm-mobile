...
adb -s "${ANDROID_SERIAL_SELECTED}" logcat -c >/dev/null 2>&1 || true
...
# Keep the combined summary format, but leave iOS unset for Android-only runs.
python3 - <<'PY' "${METRICS_DIR}/android-device.json" "${METRICS_DIR}/android.json" "${METRICS_DIR}/ios.json" "${METRICS_DIR}/device-summary.json"
import json
import pathlib
import sys
...
echo "==> Android physical-device metrics written under ${METRICS_DIR}"
